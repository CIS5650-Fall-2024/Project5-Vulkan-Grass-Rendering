...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        {
            descriptorWrites[i * 3 + j].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[i * 3 + j].dstSet = computeDescriptorSets[i];
            descriptorWrites[i * 3 + j].dstBinding = j;
            descriptorWrites[i * 3 + j].dstArrayElement = 0;
            descriptorWrites[i * 3 + j].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            descriptorWrites[i * 3 + j].descriptorCount = 1;
            descriptorWrites[i * 3 + j].pBufferInfo =
                ((j == 0) ? &inputBladesBufferInfo : (j == 1) ? &culledBladesBufferInfo : &numBladesBufferInfo);
            descriptorWrites[i * 3 + j].pImageInfo = nullptr;
            descriptorWrites[i * 3 + j].pTexelBufferView = nullptr;
        }
    }
