...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    // Binding for culled grass blades
    ...
    // Binding for the number of grass blades
    ...
    // Specify descriptor set layout details
    VkDescriptorSetLayoutCreateInfo layoutInfo = {};
    layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
    layoutInfo.pBindings = bindings.data();
...
void Renderer::CreateGrassDescriptorSets() {
    ...
    // If the allocation fails, throw an error.
    if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, grassDescriptorSets.data()) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate descriptor set");
    }

    // Prepare data to be written to the descriptor sets for each group of grass blades.
    std::vector<VkWriteDescriptorSet> descriptorWrites(grassDescriptorSets.size());

    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        // Describe the buffer info for the model data for each blade group.
        VkDescriptorBufferInfo modelBufferInfo = {};
        modelBufferInfo.buffer = scene->GetBlades()[i]->GetModelBuffer();
        modelBufferInfo.offset = 0;
        modelBufferInfo.range = sizeof(ModelBufferObject);

        // Set up the write operation details for updating the descriptor set.
    ...
    // Perform the descriptor set updates with the prepared write operations.
    vkUpdateDescriptorSets(logicalDevice, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
}
...
void Renderer::CreateComputeDescriptorSets() {
  ...
  for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      descriptorWrites[3 * i + j] = {};
      descriptorWrites[3 * i + j].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
      descriptorWrites[3 * i + j].dstSet = computeDescriptorSets[i];
      descriptorWrites[3 * i + j].dstBinding = j;
      descriptorWrites[3 * i + j].dstArrayElement = 0;
      descriptorWrites[3 * i + j].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
      descriptorWrites[3 * i + j].descriptorCount = 1;
      descriptorWrites[3 * i + j].pImageInfo = nullptr;
      descriptorWrites[3 * i + j].pTexelBufferView = nullptr;
    }
...
void Renderer::RecordComputeCommandBuffer() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        vkCmdDispatch(computeCommandBuffer, static_cast<uint32_t>((NUM_BLADES + WORKGROUP_SIZE - 1) / WORKGROUP_SIZE), 1, 1);
    }
