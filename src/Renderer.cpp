...
void Renderer::CreateTimeDescriptorSetLayout() {
    ...
    uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    uboLayoutBinding.descriptorCount = 1;
    uboLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; // only access in compute stage
    uboLayoutBinding.pImmutableSamplers = nullptr;
...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings = { uboLayoutBinding, uboLayoutBinding2, uboLayoutBinding3 };
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetModels().size(); ++i) {
        ...
        VkDescriptorBufferInfo computeBufferInfo1 = {};
        computeBufferInfo1.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
        computeBufferInfo1.offset = 0;
        computeBufferInfo1.range = NUM_BLADES * sizeof(Blade);

        VkDescriptorBufferInfo computeBufferInfo2 = {};
        computeBufferInfo2.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
        computeBufferInfo2.offset = 0;
        computeBufferInfo2.range = sizeof(BladeDrawIndirect);
        ...
        descriptorWrites[3 * i + 1].pBufferInfo = &computeBufferInfo1;
        ...
        descriptorWrites[3 * i + 2].pBufferInfo = &computeBufferInfo2;
