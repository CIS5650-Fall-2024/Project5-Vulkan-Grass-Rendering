...
void main() {
    ...
    const float w = tes_v2[0].w;
    

#if DEBUG_QUADS
    const vec3 n = vec3(1, 0, 0);

    const vec3 c0 = v0 - vec3(1, 0, 0) + v * vec3(0, 3, 0);
    const vec3 c1 = v0 + vec3(1, 0, 0) + v * vec3(0, 3, 0);
#else
    const vec3 a = v0 + v * (v1 - v0);
    const vec3 b = v1 + v * (v2 - v1);
    const vec3 c = a + v * (b - a);
    const vec3 t1 = vec3(cos(theta), 0, sin(theta));
    const vec3 c0 = c - w * t1;
    const vec3 c1 = c + w * t1;
