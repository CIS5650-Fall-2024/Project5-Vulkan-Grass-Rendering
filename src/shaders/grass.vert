...
void main() {
    ...
    v0_o = model * vec4(v0_i.xyx, 1.f);
    gl_Position = v0_o;
    v0_o.w = v0_i.w;

    v1_o = vec4((model * vec4(v1_i.xyx, 1.f)).xyz, v1_i.w);
    v2_o = vec4((model * vec4(v2_i.xyx, 1.f)).xyz, v2_i.w);
    up_o = vec4((model * vec4(up_i.xyx, 1.f)).xyz, up_i.w);
}
